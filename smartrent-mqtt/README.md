...
A [timoni.sh](http://timoni.sh) module for deploying smartrent-mqtt to Kubernetes clusters.
...
```shell
timoni -n default apply smartrent-mqtt oci://philipsd6/timoni-smartrent-mqtt
```
...
```shell
timoni -n default apply smartrent-mqtt oci://philipsd6/timoni-smartrent-mqtt --values ./my-values.cue
```
...
```shell
timoni -n default delete smartrent-mqtt
```
...
| `image: repository:`                 | `string`                         | `docker.io/smartrent-mqtt` | Container image repository                                              |
...
| `bitwarden: id:`                     | `string`                         | `""`                  | If set, will create an ExternalSecret using this Bitwarden item         |
| `bitwarden: source:`                 | `string`                         | `fields`              | One of `fields`, `note`, `attachments`, `login`                         |
| `bitwarden: variables:`              | `[...string]`                    | `[]`                  | If set, will set secretKey from remote property of the same name        |
| `bitwarden: mapping:`                | `{[string]: string}`             | `{}`                  | If set, will map secretKey to a remote property                         |
| `bitwarden: items:`                  | `[...{source: string, secretKey: string, remoteProperty: string}]` | `[]` | If set, will set secretKey and remoteProperty from the specified source |
